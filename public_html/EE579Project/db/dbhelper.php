...
function insertResponse($surveyId, $userId, $responseArray) {
	...
	if (!$conn) {
		...
		$sql = "INSERT INTO survey_response (response_id, question_id, rel_question_id, question_type, response, survey_id, email_id, timestamp) VALUES ";
		...
		for ($i = 0; $i < count($responseArray); $i++) {
			...
			if ($i < (count($responseArray) - 1)) {
				$sql .= "(DEFAULT, $question_id, $rel_question_id, '$qType', '$response', $survey_id, '$userid', '$timestamp'), ";
			} else {
				$sql .= "(DEFAULT, $question_id, $rel_question_id, '$qType', '$response', $survey_id, '$userid', '$timestamp') ";
			}
		}
...
function exportSurveyDataToCSVInTimeRange($user_id, $start_time, $end_time) {
	include ('database.php');
	$conn = openDB();
	if (!$conn) {
		echo 'open db fail';
		...
		mysql_select_db($db, $conn);
		$query = "SELECT * FROM survey_response WHERE user_id = '$user_id'";
		$result = mysql_query($query);
	
		$csvString = "";
	
		$res = mysql_query("SELECT * FROM survey_response WHERE email_id = '$user_id' AND timestamp >= '$start_time' AND timestamp <= '$end_time'");
...
function exportContexualToCSVInTimeRange($user_id, $start_time, $end_time) {
	include ('database.php');
	$conn = openDB();
	if (!$conn) {
		echo 'open db fail';
		...
		mysql_select_db($db, $conn);
		$query = "SELECT * FROM interval_data WHERE userid = '$user_id' AND timestamp >= '$start_time' AND timestamp <= '$end_time'";
		$result = mysql_query($query);
	
		$csvString = "";
	
		$res = mysql_query("SELECT * FROM interval_data WHERE userid = '$user_id' AND timestamp >= '$start_time' AND timestamp <= '$end_time'");
...
function exportInteractionToCSVInTimeRange($user_id, $start_time, $end_time) {
	include ('database.php');
	$conn = openDB();
	if (!$conn) {
		echo 'open db fail';
		...
		mysql_select_db($db, $conn);
		$query = "SELECT * FROM push_type WHERE email_id = '$user_id' AND timestamp >= '$start_time' AND timestamp <= '$end_time'";
		$result = mysql_query($query);
	
		$csvString = "";
	
		$res = mysql_query("SELECT * FROM push_type WHERE email_id = '$user_id' AND timestamp >= '$start_time' AND timestamp <= '$end_time'");
...
function insertIntoContexualData($userid, $light, $temp, $battery, $motion, $S4_temp, $wifi) {
	include ('database.php');
	$status = array();
	$conn = openDB();
	...
	if (!$conn) {
		...
		$hashUserId = hashString($userid);
		$timestamp = time();
		$sql = "INSERT INTO interval_data " . "(timestamp,userid, light, temparature, battery, motion, S4_temperature, wifi) " . "VALUES($timestamp, '$userid', $light, $temp, $battery, $motion, $S4_temp, '$wifi')";
		//echo "$sql";
...
function insertIntoCapeData($device, $temp, $humidity) {
	include ('database.php');
	$status = array();
	$conn = openDB();
	...
	if (!$conn) {
		...
		$timestamp = time();
		$sql = "INSERT INTO cape_data " . "(device, timestamp, temperature, humidity) " . "VALUES('$device', $timestamp, $temp, $humidity)";
		//echo "$sql";
...
function insertSurveyPushSet($name, $emailId, $frequency, $light, $temperature, $group_id) {
	include ('database.php');
	$status = array();
	$conn = openDB();
	...
	if (!$conn) {
		...
		$sql = "INSERT INTO survey_push_set " . "(survey_id, email_id, survey_freq, ligth_thre, temp_thre, group_name) " . "VALUES('$name','$emailId','$frequency',$light,'$temperature','$group_id')";
...
function insertNewSurvey($name, $title, $location, $category, $desc, $segmentParams) {
	...
	if (!$conn) {
		...
		if (!$retval) {
			...
			if ($status['code'] > 0) {
				$status = array('code' => mysql_affected_rows($conn), 'message' => 'Survey successfully added. Now it is time to populate the questions\nwith using the Add Questions Tab.');
			} else {
				$status = array('code' => -1, 'message' => $status['message']);
			}

			assignSurveyUsers($survey_id);
                        assignUserDefaultSurvey($survey_id, $category);
		}
...
function assignUserDefaultSurvey($survey_id, $group_id) {
	include ('database.php');
	$status = array();
	$conn = openDB();
	...
	if (!$conn) {
		...
		$sql = "UPDATE group_user SET default_survey = '$survey_id' WHERE group_name = '$group_id'";
		...
		if (!$retval) {
			...
			$status = array('code' => -1, 'message' => 'Survey has been pushed to ' . $group_id . ' group');
		}
...
function insertNewPushSurvey($group_id, $email_id, $name, $push_type, $push_duration) {
	include ('database.php');
	$status = array();
	$conn = openDB();
	...
	if (!$conn) {
		...
		$sql = "UPDATE group_user SET push_survey_id = '$name', push_type = '$push_type', push_duration='$push_duration' WHERE group_name = '$group_id' AND email_id = '$email_id'";
...
function setDefaultSurvey($group_id, $email_id, $name) {
	include ('database.php');
	$status = array();
	$conn = openDB();
	...
	if (!$conn) {
		...
		$sql = "UPDATE group_user SET default_survey = '$name' WHERE group_name = '$group_id' AND email_id = '$email_id'";
		//"UPDATE signupkeys SET status = 0 AND userid = '$key'"
		echo $sql;
		...
		if (!$retval) {
			...
			$status = array('code' => -1, 'message' => 'Default has been set to ' . $email_id . ' user');
		}
...
function resetPushSurvey($email) {
	...
	$sql = "UPDATE group_user SET push_survey_id = 0 WHERE email_id = '$email'";
...
function fetchUser($username, $password) {
	...
	if (!$conn) {
		...
		$sql = "SELECT * FROM 579_log_in WHERE username = '$username' AND password = '$password'";
		$result = array();
		mysql_select_db($db);
		$retval = mysql_query($sql, $conn);
		...
		$row = mysql_fetch_assoc($retval);
                if (!$row) {
			$status = array('code' => -1, 'message' => 'invalid');
		} else {
...
function fetchTempHum($startdate, $enddate) {
      include ('database.php');
      $status = array();
      $conn = openDB();
      $resultArray = array();
      ...
      if (!$conn) {
            ...
            $sql = "SELECT * FROM cape_data WHERE timestamp >= '$startdate' AND timestamp <= '$enddate'";
            //echo $sql;
            $result = array();
            mysql_select_db($db);
            $retval = mysql_query($sql, $conn);
...
function insertNewQuestion($survey_id, $text, $type, $options, $rText, $rType, $rOptions, $segment, $threshold) {
	...
	if (!$conn) {
		...
		$sql = "INSERT INTO survey_questions " . "(question_id, survey_id, question_text, question_type, segment_id, threshold) " . "VALUES(DEFAULT, $survey_id,'$text', '$type', $segment, $threshold)";
...
/* Api Used Methods */
function getGroupUserData($group_id) {
	...
	if (!$conn) {
		...
		$sql = "SELECT * FROM group_user WHERE group_name = '$group_id'";
...
function getPushNotification($email) {
	...
	if (!$conn) {
		...
		$sql = "SELECT push_survey_id, push_type, default_survey, push_duration FROM group_user WHERE email_id = '$email'";
...
/* Api Used Methods */
function savePushType($push_type, $email) {
	include ('database.php');
	$status = array();
	$conn = openDB();
        $timestamp = time();
	...
	if (!$conn) {
		...
		$sql = "INSERT INTO push_type " . "(push_type, email_id, timestamp) " . "VALUES('$push_type', '$email', '$timestamp')";
		...
		$retval = mysql_query($sql, $conn);
                ...
                if (!$retval) {
			$status = array('code' => -1, 'message' => 'Error');
                        echo "error";
		} else {
			$status = array('code' => -1, 'message' => 'Group successfully added. Now it is time to populate the group \nwith email addresses using the Manage User Tab.');
		}
...
function getUserAsGroup($group_id) {
	include ('database.php');
	$status = array();
	$conn = openDB();
...
function getSurveyAsGroup($group_id) {
	include ('database.php');
	$status = array();
	$conn = openDB();
	...
	if (!$conn) {
		...
		$sql = "SELECT * FROM survey_master WHERE category_id = '$group_id'";
		//echo "$sql";
		$result = array();
		mysql_select_db($db);
		$retval = mysql_query($sql, $conn);
...
function deleteExistingSurvey($delete_survey_id) {
	...
	if (!$conn) {
		...
		if (!$retval) {
			...
			$status = array('code' => -1, 'message' => 'Survey successfully deleted with Survey ID: ');
		}
...
function deleteGroup($delete_group) {
	...
	if (!$conn) {
		...
		$sql = "DELETE FROM group_user WHERE group_name = '$delete_group'";
		...
		if (!$retval) {
			$status = array('code' => -1, 'message' => mysql_error());
                ...
                $sql = "DELETE FROM survey_group WHERE group_name = '$delete_group'";
		...
		if (!$retval) {
			$status = array('code' => -1, 'message' => 'Error occurred.');
		} else {
			$status = array('code' => -1, 'message' => 'Group successfully deleted with group name: ');
		}
...
function deleteSurveyRelOptions($delete_survey_id) {
	...
	if (!$conn) {
		...
		$sql = "DELETE FROM survey_rel_questions_options WHERE survey_id = '$delete_survey_id'";
		//$sql = "DELETE FROM contextual_data";
		echo $sql;
